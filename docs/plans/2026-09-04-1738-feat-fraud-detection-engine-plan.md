...
> **Nota de preservação:** Os significados e identificadores R/A/F/AE definidos no brainstorm foram preservados. As escolhas de tecnologia, modelos de dados e mecanismos de execução pertencem ao contrato de planejamento.
...
- A2. **Pessoa analista ou aprovadora de regras:** cria, revisa, aprova, ativa e retira versões de regras conforme sua autorização.
...
- R8. Alterações de regra devem seguir `DRAFT`, `PENDING_APPROVAL`, `APPROVED`, `ACTIVE` e `RETIRED`, mantendo versão imutável, autoria, aprovação e trilha de auditoria.
- R9. A mesma pessoa não deve aprovar a própria alteração de regra.
- R10. O motor deve trocar o conjunto ativo de regras atomicamente para novas avaliações.
- R11. Uma indisponibilidade do gerenciamento deve manter o último conjunto válido em execução.
- R12. Uma instância sem conjunto válido de regras não deve ficar pronta.
...
stateDiagram-v2
  ...
  PENDING_APPROVAL --> DRAFT: rejeitada
  PENDING_APPROVAL --> APPROVED: aprovador diferente do autor
  APPROVED --> ACTIVE
  ACTIVE --> RETIRED
```
...
- R14. O motor deve deduplicar reentregas por `eventId` sem repetir efeitos de negócio.
...
- R17. A atomicidade oferecida pelo processador deve ser habilitada quando preservar o SLO definido em R27.
...
- R33. SRE deve observar métricas, logs estruturados e rastros correlacionados para disponibilidade, vazão, latência, atraso de consumo, erros, regras acionadas, `INCONCLUSIVE`, duplicidades, falhas de dependência e recuperação do acúmulo.
...
- F4. **Mudança de regra**
  ...
  - **Atores:** A2, A3, A9.
  - **Etapas:** Uma segunda pessoa aprova, a versão é ativada e o motor troca atomicamente o conjunto usado em novas avaliações.
...
- AE3. **Reentrega idêntica.** **Abrange R14, R16.** Dado o mesmo `eventId` recebido mais de uma vez, quando as entregas são processadas, então ocorre somente o conjunto de efeitos da primeira avaliação; alerta e notificação existem apenas quando essa avaliação é suspeita.
...
- KTD8. **IDs separados para o fluxo online e o backtest.** No fluxo online, `assessmentId` será determinístico a partir de `eventId`; `alertId` derivará de `assessmentId`; e `notificationRequestId` derivará de `alertId` e canal. Um backtest usará `runId`, `eventId` e `rulesetVersion` em espaço de nomes próprio para não colidir com a avaliação online. Rege R4, R14, R16, R18, R38.
...
- KTD10. **Snapshot completo, inicialização bloqueante e propagação observável.** Cada ativação publicará um `RuleSetSnapshot` imutável com versão monotônica, hash, autor e aprovador em tópico compactado. Antes de iniciar a topologia, um consumidor de inicialização carregará pelo menos um snapshot válido; sem ele, os offsets de transação permanecerão intocados. Cada avaliação capturará uma única versão completa. A troca é atômica por avaliação, não um corte simultâneo global entre instâncias; a versão carregada será exposta por instância. Rege R8, R10, R11, R12, R13, R33.
- KTD11. **PostgreSQL e outbox transacional no plano de controle.** (decisão consolidada na sessão: aprovada pelo usuário — escolhida em vez de MongoDB ou DynamoDB porque transações relacionais, restrições e JSONB atendem ao fluxo, à auditoria e à outbox sem adicionar outro modelo de persistência.) A mudança de estado, auditoria e outbox serão gravadas na mesma transação. A ativação será assíncrona até o publicador confirmar a publicação. Rege R8, R9, R10, R11, R29, R31.
- KTD12. **Estado do caminho crítico em RocksDB com log de alterações Kafka.** Repositórios separados manterão integridade e deduplicação, histórico temporal por cliente e último conjunto de regras válido. O horizonte inicial será 24 horas para identidade, 10 minutos para a maior janela e 15 minutos para retenção do histórico. Esses valores são demonstrativos e configuráveis. Rege R14, R15, R19, R20, R21, R26.
- KTD13. **Tempo do evento sem espera artificial.** `occurredAt` será o timestamp do registro. A tolerância inicial de atraso será de 2 minutos, a tolerância de futuro será de 1 minuto e eventos não serão retidos aguardando reordenação. Eventos atrasados consultarão somente fatos com horário menor ou igual ao seu e depois contribuirão para avaliações futuras. Essa interpretação protege o SLO e preserva R20-R22. Rege R20, R21, R22, R27.
- KTD14. **Tabela formal de agregação dos resultados.** Qualquer `MATCHED` produz `SUSPICIOUS`; sem correspondência e com pelo menos um `NOT_EVALUATED` produz `INCONCLUSIVE`; todas as regras aplicáveis em `NO_MATCH` produzem `NOT_SUSPICIOUS`. Evento estruturalmente inválido não é aceito e não produz avaliação. Rege R3, R13, R21, R23.
- KTD15. **Caixa de entrada e registro de entregas idempotentes no notificador.** O consumidor persistirá uma solicitação `PENDING` com unicidade por `notificationRequestId` antes de confirmar o offset; um despachante separado assumirá pendências por concessão temporária. O adaptador SMTP usará `Message-ID` determinístico e consultará o Mailpit antes de repetir uma tentativa `SENDING` ambígua. Em produção, o canal deverá oferecer chave de idempotência ou consulta de status; SMTP puro não fornece processamento exatamente uma vez. Rege R6, R16, R18, R23, R24.
...
flowchart TB
  ...
  Operator[Autor / aprovador] -->|JWT + REST| Control[Serviço de controle de fraude]
  Control -->|regras, auditoria, outbox| RulesDb[(PostgreSQL<br/>rules schema)]
  RulesDb --> Relay[Publicador da outbox]
...
```mermaid
flowchart TB
  Start[Registro recebido] --> Schema{Schema e limites válidos?}
  ...
  Identity -->|novo| RuleSet{Ruleset válido carregado?}
  RuleSet -->|não| NotReady[Topologia não inicia<br/>offset permanece intacto]
  RuleSet -->|sim| Evaluate[Avaliar regras com tempo do evento]
...
#### Ativação e propagação de regras
...
sequenceDiagram
  ...
  participant R as Publicador da outbox
  participant K as Tópico de regras
  participant E as Instâncias do motor

  A->>C: criar e submeter versão da regra
  C->>D: persistir rascunho e auditoria
  V->>C: aprovar com sujeito diferente
  C->>D: aprovar, montar snapshot e incluir auditoria/outbox atomicamente
  C-->>V: ativação aceita e publicação pendente
  R->>D: assumir registro não publicado da outbox
  R->>K: publicar RuleSetSnapshot monotônico
  R->>D: marcar registro da outbox como publicado
  K-->>E: replicar snapshot compactado
  E->>E: validar hash, versão e limites da DSL
  E->>E: substituir snapshot local completo
  Note over E: versão carregada observável por instância
```
...
| `CustomerNotificationRequested` | IDs da solicitação/alerta, referências opacas de cliente/transação, categoria segura, ID do modelo, localidade | chave `notificationRequestId` | Não contém evidência de regra nem contato. |
...
| `RuleSetSnapshot` | ID/versão/hash do snapshot, autor, aprovador, `activatedAt`, lista completa de regras imutáveis | chave `ACTIVE` no tópico de regras | A versão válida mais alta substitui o snapshot local. |
...
|---|---|---:|---|---|---|
| `fraud.transaction.received.v1` | `transactionId` | 12 | exclusão, 24 h | produtor/ferramenta de carga | motor de detecção |
...
erDiagram
  ...
  RULE_VERSION }o--o{ RULESET_SNAPSHOT : incluida_em
...
O `fraud-control-service` oferecerá operações REST versionadas para criar, consultar, submeter, aprovar, ativar e retirar versões, além de consultar o status desejado e publicado do ruleset. A ativação retornará aceitação assíncrona porque PostgreSQL e Kafka não compartilham uma transação.

| Permissão | Capacidade |
|---|---|
| `RULE_READ` | Consultar regras, versões e status de publicação. |
| `RULE_WRITE` | Criar draft e submeter versão válida. |
| `RULE_APPROVE` | Aprovar versão criada por outro `sub`. |
| `RULE_ACTIVATE` | Ativar snapshot aprovado ou retirar regra via novo snapshot. |
...
|---|---|---|
| API REST de regras | token JWT assinado, validação de emissor/audiência/expiração, RBAC e separação entre autor e aprovador | provedor corporativo de identidade, MFA para pessoas, tokens de curta duração e acesso emergencial auditado |
...
| Corridas do conjunto de regras entre instâncias | Transações adjacentes usam versões diferentes | Snapshot por avaliação, versão monotônica, último válido conhecido e métrica da versão carregada; não prometer troca global simultânea. |
...
- **Requisitos:** R1, R2, R5, R6, R28, R30, R31, R32, R35, R37; KTD1, KTD2, KTD4, KTD5.
...
- **Requisitos:** R7, R8, R9, R10, R11, R12, R29, R31, R32, R35; F4; AE5, AE6; KTD9, KTD10, KTD11, KTD18.
- **Dependências:** U1.
- **Arquivos:**
...
- **Abordagem:**
  1. Modelar transições do ciclo de vida como operações de domínio com bloqueio otimista; rejeitar transições ilegais e desatualizadas antes da persistência.
  2. Validar a AST segura das regras e seus limites de complexidade na criação, aprovação e carga do snapshot.
  3. Montar um snapshot completo e imutável para ativação, em vez de alterar uma regra ativa no mesmo lugar.
  4. Persistir transição, evento de auditoria, snapshot e entrada da outbox em uma transação PostgreSQL.
  5. Publicar registros da outbox pelo menos uma vez; tornar idempotente a atualização do tópico de regras usando o ID do snapshot e a versão monotônica.
  6. Validar declarações e permissões do JWT assinado e, depois, impor `approver.sub != author.sub` independentemente da sobreposição de papéis.
...
- **Cenários de teste:**
  - Uma pessoa com permissão de escrita cria e submete um rascunho válido; outro sujeito autorizado o aprova e ativa.
  ...
  - Operador inválido, profundidade excessiva da AST, número excessivo de regras ou janela além do máximo configurado é rejeitado antes da ativação.
  - Duas aprovações ou ativações concorrentes resultam em uma transição confirmada e um conflito de estado desatualizado.
  ...
  - JWT com assinatura, emissor, audiência ou expiração inválida é rejeitado; um token válido sem a permissão necessária é negado.
  - Um snapshot com versão inferior, alterado ou com hash inválido não pode ser emitido como a versão ativa mais recente.
...
- **Verificação:** Solicitações autorizadas do ciclo de vida criam um snapshot auditado, a outbox eventualmente publica uma versão canônica do conjunto de regras e solicitações negadas ou concorrentes não corrompem o estado.
...
- **Cenários de teste:**
  ...
  - Reavaliar o mesmo evento online produz o mesmo ID de avaliação e, quando suspeito, os mesmos IDs de alerta e notificação.
...
- **Requisitos:** R1-R5, R10-R27, R31-R33, R35; F1, F2, F3, F5, F6; AE1-AE4, AE6, AE8, AE9; KTD3, KTD6-KTD8, KTD10, KTD12-KTD14, KTD17.
...
- **Abordagem:**
  ...
  3. Materializar repositórios persistentes nomeados para identidade da transação, impressões digitais dos eventos, histórico do cliente e snapshots do conjunto de regras, com logs de alterações habilitados.
  4. Desabilitar o início automático da topologia, ler o snapshot compactado válido mais recente por um consumidor de inicialização e iniciar Kafka Streams somente depois que essa etapa tiver sucesso.
  5. Continuar as atualizações do conjunto de regras pelo repositório global, validar versão monotônica/hash/DSL e manter o último conjunto válido conhecido diante de atualizações inválidas.
...
- **Cenários de teste:**
  ...
  - Abrange AE3. O mesmo evento/chave/impressão digital recebido repetidamente não produz nova saída nem alteração de estado depois da primeira confirmação.
  ...
  - Abrange AE6. A ausência de conjunto de regras inicial mantém a prontidão negativa; a indisponibilidade do plano de controle depois de um snapshot válido não interrompe a avaliação.
  ...
  - Um conjunto de regras com versão inferior, hash incorreto ou conteúdo inválido mantém o último conjunto válido carregado e incrementa uma métrica operacional de rejeição.
  ...
  - Um teste de integração com duas instâncias mostra que cada avaliação registra uma versão completa do conjunto de regras, enquanto a convergência da versão carregada permanece observável.
...
- **Abordagem:**
  ...
  4. Aplicar espera exponencial limitada e circuit breaker aos adaptadores de cadastro e canal sem afetar os alertas internos.
  5. Usar `Message-ID` determinístico; reconciliar com Mailpit antes de uma nova tentativa após timeout/falha ambígua.
  6. Publicar resultados de entrega sanitizados e encaminhar falhas esgotadas para a DLQ de notificação.
...
- **Abordagem:**
  ...
  4. Provisionar um painel e regras de alerta acionáveis para SLO, atraso de consumo, restauração, conjunto de regras/outbox e saúde das notificações.
...
- **Cenários de teste:**
  ...
  - Um operador mal-intencionado de regras não consegue ativar expressões executáveis nem um conjunto de regras além dos limites de CPU/estado.
...
- **Requisitos:** R1-R6, R10-R12, R14-R25, R31-R37; F1-F6; AE1-AE10, AE12; KTD17, KTD20.
...
- **Arquivos:**
  ...
  - `tools/system-tests/tests/test_rule_activation.py`
...
- **Abordagem:**
  1. Inicializar dois sujeitos JWT locais, cadastrar um conjunto de regras pela API pública e aguardar a prontidão da versão carregada.
  2. Produzir entradas de caixa-preta e consumir todas as saídas com isolamento de dados confirmados, em vez de inspecionar detalhes internos do Java.
  3. Reiniciar serviços e interromper dependências em pontos definidos e, depois, reconciliar resultados pela identidade determinística.
  4. Manter a verificação de fumaça rápida e determinística; manter experimentos destrutivos/de recuperação em um perfil explícito de testes de sistema.
  5. Validar o contrato de backtest e o caminho de negação sem construir o pipeline completo de reprocessamento.
...
- **Cenários de teste:**
  ...
  - Uma identidade de backtest não consegue alcançar o tópico nem o caminho de consumo das notificações online.
  - Abrange AE12. Um consumidor interno alternativo e compatível lê alertas sem alterar a lógica do motor.
- **Verificação:** Um comando documentado executa o fluxo saudável de fumaça; uma suíte separada de resiliência registra recuperação, escoamento do acúmulo e contagens de efeitos para cada falha injetada.
...
- **Arquivos:**
  ...
  - `docs/ai-usage.md`
- **Abordagem:**
  1. Tornar o README o caminho mais curto entre um checkout limpo e um resultado ponta a ponta visível.
  2. Copiar deste plano o desenho arquitetural de referência e atualizá-lo para corresponder aos nomes implementados e ao comportamento medido.
  3. Registrar cada alternativa e trade-off adotado, incluindo detecção assíncrona, Kafka Streams, DSL segura, fronteiras do PostgreSQL, JSON Schema e ausência de modelo de leitura.
  4. Documentar o modelo de ameaças e cada fronteira de segurança entre ingestão e alerta, separando controles locais executáveis dos controles de produção. Mapear cada cenário de abuso para ator, ativo, fronteira de confiança, controle responsável e evidência de verificação.
  5. Publicar estratégia de testes, manual operacional, medição do SLO, método de capacidade, evidências de benchmark, limitações e evoluções.
  6. Declarar onde a IA auxiliou ideação, pesquisa, planejamento, código, testes e documentação, além de como as saídas foram revisadas e verificadas pelo autor.
  7. Executar verificações unitárias/de contrato/estáticas no CI normal; executar testes de integração com Docker em uma tarefa dedicada; manter carga e caos como artefatos manuais ou agendados.
...
| Qualidade Python | `python -m pytest tools/system-tests tools/load-generator/tests` e `python -m ruff check tools` | U7, U8 | Auxiliares de caixa-preta e cálculos de carga passam sem serviços externos onde houver simulação. |
...
| Verificação de fumaça | `./scripts/smoke.sh` | U2-U7 | Uma ativação do conjunto de regras, avaliação normal, alerta suspeito e um e-mail terminam com sucesso. |
...
| Segurança/privacidade | suítes de autorização e marcadores sintéticos | U2, U4-U7 | Identidades inválidas são negadas e marcadores de dados proibidos não escapam de sua fronteira. |
...
- Cada evento único aceito possui exatamente uma avaliação online; eventos suspeitos possuem um alerta consolidado e uma solicitação de notificação.
- Regras podem ser criadas, aprovadas de forma independente e ativadas sem nova implantação da aplicação, com auditoria e propagação confiável do snapshot.
...
|---|---|
| U1 | Build limpo, schemas válidos e infraestrutura central saudável a partir de um checkout limpo. |
| U2 | Ciclo de vida governado, JWT/RBAC, auditoria/outbox e publicação do conjunto de regras passam nos testes de concorrência e falha. |
