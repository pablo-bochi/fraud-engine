...
class NotificationReplayIntegrationTest {
  ...
  static final PostgreSQLContainer<?> POSTGRES =
    ...
    static final GenericContainer<?> MAILPIT =
        new GenericContainer<>(
                DockerImageName.parse("axllent/mailpit:v1.31.0"))
    ...
    try (ConfigurableApplicationContext application =
        new SpringApplicationBuilder(
            ...
            .run(
                ...
                "--spring.kafka.consumer.group-id=notification-replay-"
                    + UUID.randomUUID(),
      ...
      assertThat(results.getFirst().providerReference())
          .isNotBlank();
  ...
  private static void publishRequest(
      CustomerNotificationRequested request) throws Exception {
  ...
  private static List<NotificationResult> readResults(int expected)
      throws Exception {
    ...
    if (results.size() != expected) {
      throw new AssertionError(
          "EXPECTED_"
              + expected
              + "_NOTIFICATION_RESULTS_BUT_OBSERVED_"
              + results.size());
    }
  ...
  private static JsonNode awaitMailpitMessages(int expected)
      throws Exception {
    ...
    throw new AssertionError(
        "EXPECTED_" + expected + "_MAILPIT_MESSAGES");
  }
